...
OUTPUT_DIR = "G:/seidataforCIL2"  # 处理后数据保存根目录
...
def extract_features(iq_file_path):
    """从IQ文件中提取特征：[实部, 实部绝对值, 实部FFT绝对值]"""
    try:
    # 加载IQ信号（复数数组）
        iq_data = np.load(iq_file_path) # if not np.iscomplexobj(iq_data): # raise ValueError(f"文件 {iq_file_path} 不是复数IQ信号") # 提取实部特征
        q_part = iq_data[1, :]
        q_abs = np.abs(q_part)
        q_fft = np.fft.fft(q_part)
        fft_abs = np.abs(q_fft)
        features = np.column_stack((q_part, q_abs, fft_abs))
        return features
